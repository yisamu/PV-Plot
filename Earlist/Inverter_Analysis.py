# -*- coding: utf-8 -*-
"""
Created on Sat Sep 1 14:55:11 2023
...
plt.ylabel('AC AMP')
plt.figure(figsize=(12,3))
plt.plot(data['Inv 19', 'DC W'] / data['Inv 19', 'DC VTG'])
...
# # paint inverter_efficiency data
# plt.figure(figsize=(10, 6))
# for i in range(len(IE)):
#     plt.plot(IE.iloc[i], label=f'Inverter {i+1}')
# plt.xlabel('Day of Year')
# plt.ylabel('efficiency')
# plt.title('Inverter Efficiency Over Days')
# # plt.legend()
# plt.show()


# paint inverter_efficiency data ECDF curve maybe ignore
sns.set_style("darkgrid")
...
for i in range(40):
    if i==3:
        continue
    name = 'Inv ' + str(i + 1)
    ax = sns.ecdfplot(data = inverter_efficiency[i])  #ECDF curve
plt.xlim(-0.1, 1.02)
...
# watch the inverter_efficiency[0]
plt.figure(figsize=(6,4))
...
ax.fill_between(ave_temp['Days'], Temp_AMB.max(axis=1), Temp_AMB.min(axis=1), color='b', alpha=.1) # fill-shadow
plt.tight_layout()
# plt.title("Daily Amb Temp Curve / degC")
plt.show()
...
plt.tight_layout()
# plt.title("Daily POA Curve / degC")
plt.show()
...
plt.figure(figsize=(10,6))
sns.histplot(data = data[name]['PM1 Temp'].loc[starttime:endtime], color=palette[0], edgecolor=palette[0],  kde=True, alpha = 0.5, label="PM1 Temp")
...
sns.histplot(data = data[name]['PM4 Temp'].loc[starttime:endtime], color=palette[2], edgecolor=palette[2], kde=True, alpha = 0.5, label="PM3 Temp")
