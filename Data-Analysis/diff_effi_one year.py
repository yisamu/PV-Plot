...
# caculate every month  no consider the time range
...
endtime = '2022-07-31'
file_name1 = 'efficiency_07.svg'
file_name2 = 'average_efficiency_07.svg'
...
for i, day in enumerate(date_range):
    ...
    efficiency_df.dropna(subset=['Efficiency (%)'], inplace=True)
...
# end_month = date_range[-1].strftime('%B')
...
plt.title(f'Daily Average Efficiency for {current_month} ')
plt.xlabel('Date')
plt.ylabel('Average Efficiency (%)')
plt.xticks(rotation=45, fontsize=7)
plt.grid(True)
